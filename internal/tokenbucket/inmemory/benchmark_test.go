...
func BenchmarkTokenBucketInMemory_Allow(b *testing.B) {
	ctx := context.Background()
	baseIdentifier := "benchUserTBInMem"
	...
	for _, config := range configs {
		b.Run(config.name, func(b *testing.B) {
			limiter := tbinmemory.NewLimiter("bench_tb_inmemory_"+config.name, config.rate, config.capacity)
			b.ReportAllocs()
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
